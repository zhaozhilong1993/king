...
class KingIdentifier(collections.Mapping):
    ...
    def from_arn(cls, arn):
        """Generate a new KingIdentifier by parsing the supplied ARN."""
        fields = arn.split(':')
        ...
        if fields[1] != 'openstack' or fields[2] != 'heat' or not path:
            raise ValueError(_('"%s" is not a valid King ARN') % arn)
    ...
    def from_arn_url(cls, url):
        """Generate a new KingIdentifier by parsing the supplied URL.
...
class ResourceIdentifier(KingIdentifier):
    ...
    def __getattr__(self, attr):
        ...
        return KingIdentifier.__getattr__(self, attr)

    def stack(self):
        """Return a KingIdentifier for the owning stack."""
        return KingIdentifier(self.tenant, self.stack_name, self.stack_id,
                              '/'.join(self._path_components()[:-2]))


class EventIdentifier(KingIdentifier):
    ...
    def resource(self):
        """Return a KingIdentifier for the owning resource."""
